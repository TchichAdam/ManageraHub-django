...
\subsubsection{Espace Candidat (15 fonctionnalités)}
...
\begin{itemize}
    ...
    \item \textbf{SQLite / MySQL :} SGBD relationnel (SQLite utilisé pour le développement local et la production légère, avec possibilité d'évoluer vers MySQL/MariaDB grâce à l'abstraction de l'ORM Django), configuré pour garantir une intégrité transactionnelle totale.
...
\centering
\resizebox{0.95\textwidth}{!}{
\begin{tikzpicture}[classbox/.style={draw, rectangle, align=center, fill=emsiblue!5, drop shadow, rounded corners=2pt}]
    % User
    ...
    % Follow
    \node[classbox] (Follow) at (0, 13.2) [minimum width=4cm] {
    ...
    % Liaisons avec cardinalités et routage sans chevauchement
    \draw[thick, -{Stealth}] (Candidate) -- (User) node[midway, above, sloped, fill=white, inner sep=1.5pt] {{\small 1..1}};
    \draw[thick, -{Stealth}] (Company) -- (User) node[midway, above, sloped, fill=white, inner sep=1.5pt] {{\small 1..1}};
    \draw[thick, -{Stealth}] (Cert) -- (Candidate) node[midway, left, fill=white, inner sep=1.5pt] {{\small *..1}};
    \draw[thick, -{Stealth}] (App) -- (User) node[midway, left, fill=white, inner sep=1.5pt] {{\small *..1}};
    \draw[thick, -{Stealth}] (App) -- (Job) node[midway, above, fill=white, inner sep=1.5pt] {{\small *..1}};
    
    % Job to User: contourne par la droite (offset 2.0)
    \draw[thick, -{Stealth}] (Job.east) -- ++(2.0, 0) |- (User.350) node[pos=0.25, right, fill=white, inner sep=1.5pt] {{\small *..1}};
    
    % Quiz to User: contourne par la droite (offset 1.0)
    \draw[thick, -{Stealth}] (Quiz.east) -- ++(1.0, 0) |- (User.10) node[pos=0.25, right, fill=white, inner sep=1.5pt] {{\small *..1}};
    
    % Post to User: contourne par la gauche (offset -1.0)
    \draw[thick, -{Stealth}] (Post.west) -- ++(-1.0, 0) |- (User.170) node[pos=0.25, left, fill=white, inner sep=1.5pt] {{\small *..1}};
    
    \draw[thick, -{Stealth}] (Comment) -- (Post) node[midway, left, fill=white, inner sep=1.5pt] {{\small *..1}};
    
    % Comment to User: contourne par la gauche (offset -2.0)
    \draw[thick, -{Stealth}] (Comment.west) -- ++(-2.0, 0) |- (User.190) node[pos=0.25, left, fill=white, inner sep=1.5pt] {{\small *..1}};
    
    \draw[thick, -{Stealth}] (Follow) -- (User) node[midway, left, fill=white, inner sep=1.5pt] {{\small *..2}};
\end{tikzpicture}
}
...
\subsubsection{Inscription et vérification administrative d'une Entreprise}
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.9, xscale=1.35]
    % Lignes de vie
    \draw[thick] (0,0) -- (0,-6.5);
    \draw[thick] (4,0) -- (4,-6.5);
    \draw[thick] (8,0) -- (8,-6.5);
    \draw[thick] (12,0) -- (12,-6.5);
    ...
    % Messages
    \draw[->] (0,-1) -- (4,-1) node[midway, above, fill=white, inner sep=1.5pt] {{\small 1. Saisir Infos, ICE et RC}};
    \draw[->] (4,-1.5) -- (12,-1.5) node[midway, above, fill=white, inner sep=1.5pt] {{\small 2. Sauvegarder (is\_approved=False)}};
    \draw[<-] (4,-2.2) -- (12,-2.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 3. Notification confirmation}};
    \draw[->] (4,-2.8) -- (0,-2.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 4. Afficher message d'attente}};
    
    \draw[->] (8,-3.8) -- (12,-3.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 5. Consulter dossiers entreprises}};
    \draw[<-] (8,-4.4) -- (12,-4.4) node[midway, above, fill=white, inner sep=1.5pt] {{\small 6. Renvoyer les documents}};
    \draw[->] (8,-5.2) -- (12,-5.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 7. Valider (is\_approved=True)}};
    \draw[->] (12,-5.8) -- (0,-5.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 8. Mail d'activation envoyé}};
\end{tikzpicture}
\caption{Diagramme de séquence : Inscription et Vérification de l'Entreprise}
\label{fig:seq_verify_pfa2}
\end{figure}

\subsubsection{Soumission d'une candidature par un Candidat}
...
\begin{tikzpicture}[scale=0.9, xscale=1.35]
    ...
    \draw[->] (0,-1) -- (4,-1) node[midway, above, fill=white, inner sep=1.5pt] {{\small 1. Cliquer sur "Postuler"}};
    \draw[->] (4,-1.8) -- (8,-1.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 2. GET apply\_form}};
    \draw[<-] (0,-2.5) -- (4,-2.5) node[midway, above, fill=white, inner sep=1.5pt] {{\small 3. Afficher le formulaire}};
    \draw[->] (0,-3.2) -- (8,-3.2) node[midway, above, pos=0.25, fill=white, inner sep=1.5pt] {{\small 4. POST message, CV}};
    \draw[->] (8,-4) -- (12,-4) node[midway, above, fill=white, inner sep=1.5pt] {{\small 5. Enregistrer Candidature}};
    \draw[<-] (0,-5.2) -- (8,-5.2) node[midway, above, pos=0.25, fill=white, inner sep=1.5pt] {{\small 6. Rediriger (Candidature validée)}};
\end{tikzpicture}
\caption{Diagramme de séquence : Flux de candidature}
\label{fig:seq_apply_pfa2}
\end{figure}

\subsubsection{Publication d'une offre d'emploi par une Entreprise validée}
...
\begin{tikzpicture}[scale=0.9, xscale=1.35]
    ...
    % Messages
    \draw[->] (0,-1) -- (4,-1) node[midway, above, fill=white, inner sep=1.5pt] {{\small 1. Saisir \& Publier une offre}};
    \draw[->] (4,-1.8) -- (8,-1.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 2. Valider l'accès recruteur}};
    \draw[->] (8,-2.5) -- (12,-2.5) node[midway, above, fill=white, inner sep=1.5pt] {{\small 3. SELECT is\_approved FROM Company}};
    \draw[<-] (8,-3.2) -- (12,-3.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 4. Retourner True (Vérifiée)}};
    \draw[->] (8,-3.9) -- (4,-3.9) node[midway, above, fill=white, inner sep=1.5pt] {{\small 5. Autoriser la création}};
    \draw[->] (4,-4.6) -- (12,-4.6) node[midway, above, fill=white, inner sep=1.5pt] {{\small 6. Enregistrer l'offre (INSERT)}};
    \draw[<-] (4,-5.3) -- (12,-5.3) node[midway, above, fill=white, inner sep=1.5pt] {{\small 7. Confirmation (Offre créée)}};
    \draw[->] (4,-5.9) -- (0,-5.9) node[midway, above, fill=white, inner sep=1.5pt] {{\small 8. Redirection vers Tableau de bord}};
\end{tikzpicture}
\caption{Diagramme de séquence : Publication d'une offre d'emploi}
\label{fig:seq_job_pfa2}
\end{figure}

\subsubsection{Publication de post et interaction (Likes / Commentaires) sur le fil d'actualités}
...
\begin{tikzpicture}[scale=0.9, xscale=1.35]
    ...
    \draw[thick] (4,0) -- (4,-6.8);
    \draw[thick] (8,0) -- (8,-6.8);
    \draw[thick] (12,0) -- (12,-6.8);
    ...
    % Messages
    \draw[->] (0,-1) -- (4,-1) node[midway, above, fill=white, inner sep=1.5pt] {{\small 1. Utilisateur A : Publier un post}};
    \draw[->] (4,-1.6) -- (12,-1.6) node[midway, above, fill=white, inner sep=1.5pt] {{\small 2. Enregistrer Post (INSERT)}};
    \draw[<-] (4,-2.2) -- (12,-2.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 3. Retourner Post ID}};
    \draw[->] (4,-2.8) -- (0,-2.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 4. Afficher Post sur le feed global}};
    
    \draw[->] (0,-3.8) -- (8,-3.8) node[midway, above, pos=0.25, fill=white, inner sep=1.5pt] {{\small 5. Utilisateur B : Liker / Commenter}};
    \draw[->] (8,-4.4) -- (12,-4.4) node[midway, above, fill=white, inner sep=1.5pt] {{\small 6. Créer Relation (Like/Comment)}};
    \draw[<-] (8,-5.0) -- (12,-5.0) node[midway, above, fill=white, inner sep=1.5pt] {{\small 7. Retourner compteurs mis à jour}};
    \draw[->] (8,-5.6) -- (4,-5.6) node[midway, above, fill=white, inner sep=1.5pt] {{\small 8. Renvoyer données (Response JSON)}};
    \draw[->] (4,-6.2) -- (0,-6.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 9. Mise à jour dynamique HTML (AJAX)}};
\end{tikzpicture}
\caption{Diagramme de séquence : Interaction fil d'actualités social}
\label{fig:seq_social_pfa2}
\end{figure}

\subsubsection{Processus de suivi (Follow / Unfollow) entre membres du réseau}
...
\begin{tikzpicture}[scale=0.9, xscale=1.35]
    ...
    % Messages
    \draw[->] (0,-1) -- (4,-1) node[midway, above, fill=white, inner sep=1.5pt] {{\small 1. Visiter profil du Candidat B}};
    \draw[->] (4,-1.6) -- (12,-1.6) node[midway, above, fill=white, inner sep=1.5pt] {{\small 2. SELECT profil\_B FROM Database}};
    \draw[<-] (4,-2.2) -- (12,-2.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 3. Renvoyer données de B}};
    \draw[->] (4,-2.8) -- (0,-2.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 4. Afficher bouton "Suivre"}};
    \draw[->] (0,-3.6) -- (8,-3.6) node[midway, above, pos=0.25, fill=white, inner sep=1.5pt] {{\small 5. Cliquer sur "Suivre"}};
    \draw[->] (8,-4.2) -- (12,-4.2) node[midway, above, fill=white, inner sep=1.5pt] {{\small 6. Enregistrer relation (INSERT)}};
    \draw[<-] (8,-4.8) -- (12,-4.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 7. Confirmation de création}};
    \draw[->] (8,-5.4) -- (4,-5.4) node[midway, above, fill=white, inner sep=1.5pt] {{\small 8. Mettre à jour bouton à "Abonné"}};
    \draw[->] (4,-6.0) -- (0,-6.0) node[midway, above, fill=white, inner sep=1.5pt] {{\small 9. Affichage "Abonné" sur l'écran}};
\end{tikzpicture}
\caption{Diagramme de séquence : Système d'abonnement (Follow)}
\label{fig:seq_follow_pfa2}
\end{figure}

\subsubsection{Suivi en temps réel de statut de candidature (Tracker AJAX)}
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.95, xscale=1.75]
    % Lignes de vie
    \draw[thick] (0,0) -- (0,-6.8);
    \draw[thick] (4,0) -- (4,-6.8);
    \draw[thick] (8,0) -- (8,-6.8);
    ...
    \node[draw, fill=white] at (8,0.3) {BDD (SQLite/MySQL)};
    
    % Messages
    \draw[->] (0,-1) -- (4,-1) node[midway, above, fill=white, inner sep=1.5pt] {{\small 1. Polling AJAX automatique (12s)}};
    \draw[->] (4,-1.8) -- (8,-1.8) node[midway, above, fill=white, inner sep=1.5pt] {{\small 2. SELECT app\_status FROM JobApplication}};
    \draw[<-] (4,-2.6) -- (8,-2.6) node[midway, above, fill=white, inner sep=1.5pt] {{\small 3. Retourner l'état et l'historique}};
    \draw[->] (4,-3.4) -- (0,-3.4) node[midway, above, fill=white, inner sep=1.5pt] {{\small 4. Renvoyer données JSON}};
    ...
    \draw[->] (0,-4.5) -- (0,-5.2) node[midway, right, fill=white, inner sep=1.5pt] {{\small 5. Activer l'overlay de scan radar}};
    \draw[->] (0,-5.9) -- (0,-6.2) node[midway, right, fill=white, inner sep=1.5pt] {{\small 6. Re-rendre le timeline et flasher}};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.85]
    ...
    \draw[thick] (3.0,0) -- (3.0,-7.5);
    \draw[thick] (7.0,0) -- (7.0,-7.5);
    \draw[thick] (11.5,0) -- (11.5,-7.5);
    
    % En-têtes
    \node[draw, fill=lightgray!20, minimum height=0.6cm] at (0,0.3) {Acteur};
    \node[draw, fill=lightgray!20, minimum height=0.6cm] at (3.0,0.3) {Client};
    \node[draw, fill=emsiblue!10, minimum height=0.6cm] at (7.0,0.3) {ManageraHub};
    \node[draw, fill=orange!10, minimum height=0.6cm] at (11.5,0.3) {Google / GitHub};
    
    % Étapes
    % 1. Clic connexion
    \draw[->, >=latex] (0,-0.6) -- (3.0,-0.6) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{1. Clic "Google / GitHub"}};
    
    % 2. Get login url
    \draw[->, >=latex] (3.0,-1.2) -- (7.0,-1.2) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{2. GET /auth/provider/login/}};
    
    % 3. Redirection
    \draw[->, >=latex] (7.0,-1.8) -- (3.0,-1.8) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{3. HTTP 302 Redirection}};
    
    % 4. Consentement page
    \draw[->, >=latex] (3.0,-2.4) -- (11.5,-2.4) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{4. GET /oauth/authorize?client\_id=...}};
    
    % 5. Consentement utilisateur
    \draw[<->, >=latex, dashed] (0,-3.2) -- (11.5,-3.2) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{5. Authentification \& Consentement}};
    
    % 6. Redirect callback
    \draw[->, >=latex] (11.5,-4.0) -- (3.0,-4.0) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{6. HTTP 302 Callback (code)}};
    
    % 7. Callback Django
    \draw[->, >=latex] (3.0,-4.6) -- (7.0,-4.6) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{7. GET /auth/provider/login/callback/?code=...}};
    
    % 8. Backend Token exchange
    \draw[->, >=latex] (7.0,-5.2) -- (11.5,-5.2) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{8. POST /oauth/token (code + secret)}};
    \draw[<-, >=latex] (7.0,-5.8) -- (11.5,-5.8) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{9. Access Token \& User Info}};
    
    % 10. Login / Creation
    \draw[->, >=latex] (7.0,-6.4) -- (7.0,-6.8) node[midway, right, font=\scriptsize, fill=white, inner sep=1.5pt] {{10. Connexion / Inscription en BDD}};
    
    % 11. Success
    \draw[->, >=latex] (7.0,-7.2) -- (3.0,-7.2) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{11. HTTP 302 Redirect (Dashboard)}};
\end{tikzpicture}
}
\caption{Diagramme de séquence : Authentification sociale via OAuth 2.0}
\label{fig:seq_oauth_pfa2}
\end{figure}
...
\begin{tikzpicture}[scale=0.85]
    ...
    \draw[thick] (3.0,0) -- (3.0,-7.0);
    \draw[thick] (6.5,0) -- (6.5,-7.0);
    \draw[thick] (10.0,0) -- (10.0,-7.0);
    \draw[thick] (13.5,0) -- (13.5,-7.0);
    
    % En-têtes
    \node[draw, fill=lightgray!20, minimum height=0.6cm] at (0,0.3) {Acteur};
    \node[draw, fill=emsiblue!10, minimum height=0.6cm] at (3.0,0.3) {Django (Vues/Signaux)};
    \node[draw, fill=emsigreen!10, minimum height=0.6cm] at (6.5,0.3) {emails.py};
    \node[draw, fill=purple!10, minimum height=0.6cm] at (10.0,0.3) {SMTP Gmail};
    \node[draw, fill=lightgray!20, minimum height=0.6cm] at (13.5,0.3) {Destinataire};
    
    % Actions
    \draw[->, >=latex] (0,-0.8) -- (3.0,-0.8) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{1. Action (ex: changement de statut)}};
    \draw[->, >=latex] (3.0,-1.6) -- (6.5,-1.6) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{2. Appel send\_html\_email(template, context)}};
    \draw[->, >=latex] (6.5,-2.4) -- (6.5,-3.0) node[midway, right, font=\scriptsize, fill=white, inner sep=1.5pt] {{3. Rendu HTML + Boutons (SITE\_URL)}};
    \draw[->, >=latex] (6.5,-3.8) -- (10.0,-3.8) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{4. EmailMultiAlternatives (TLS/Port 587)}};
    \draw[->, >=latex] (10.0,-4.6) -- (13.5,-4.6) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{5. Distribution de l'e-mail}};
    \draw[->, >=latex] (13.5,-5.6) -- (0,-5.6) node[midway, above, sloped, font=\scriptsize, fill=white, inner sep=1.5pt] {{6. Clic bouton d'action cliquable}};
    \draw[->, >=latex] (0,-6.4) -- (3.0,-6.4) node[midway, above, font=\scriptsize, fill=white, inner sep=1.5pt] {{7. Redirection vers VM sslip.io}};
\end{tikzpicture}
}
\caption{Diagramme de séquence : Système d'envoi d'e-mails de notification transactionnels}
\label{fig:seq_email_pfa2}
\end{figure}

\subsection{Diagrammes d'activité}

\subsubsection{Flux de postulation et mise à jour de statut}
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape}]
    % Navigateur Client
    \node (browser) [draw, rectangle, minimum width=3.2cm, minimum height=1.6cm, align=center, fill=lightgray!20, thick, rounded corners] at (0, 0) {
        \textbf{Navigateur Client} \\ \rule{2.8cm}{0.4pt} \\
        HTML5 / CSS3 / JS \\
        DNS : \texttt{sslip.io}
    };
    
    % Box VM Oracle Cloud
    \draw[dashed, draw=emsiblue!80, fill=emsiblue!5, thick, rounded corners] (4.1, -4.0) rectangle (14.5, 4.2);
    \node[anchor=north west, color=emsiblue!90, font=\bfseries] at (4.3, 4.0) {Instance VM Oracle Cloud (Ubuntu)};
    
    % Nginx
    \node (nginx) [draw, rectangle, minimum width=3.2cm, minimum height=1.4cm, align=center, fill=white, thick, rounded corners] at (6.5, 1.8) {
        \textbf{Serveur Web Nginx} \\ \rule{2.8cm}{0.4pt} \\
        Reverse Proxy \\
        Ports 80, 443
    };
    
    % Gunicorn
    \node (gunicorn) [draw, rectangle, minimum width=3.2cm, minimum height=1.4cm, align=center, fill=white, thick, rounded corners] at (6.5, -1.8) {
        \textbf{Gunicorn (WSGI)} \\ \rule{2.8cm}{0.4pt} \\
        Serveur d'App \\
        Port local 8000
    };
    
    % Django
    \node (django) [draw, rectangle, minimum width=3.2cm, minimum height=1.6cm, align=center, fill=emsiblue!15, thick, rounded corners] at (11.5, 0) {
        \textbf{Framework Django} \\ \rule{2.8cm}{0.4pt} \\
        Application Python \\
        WhiteNoise (Static)
    };
    
    % SQLite BDD
    \node (sqlite) [draw, rectangle, minimum width=2.6cm, minimum height=1.2cm, align=center, fill=emsigreen!10, thick, rounded corners] at (11.5, 2.5) {
        \textbf{BDD SQLite} \\ \rule{2.2cm}{0.4pt} \\
        Fichier local (\texttt{db.sqlite3})
    };
    
    % Stockage local
    \node (storage) [draw, rectangle, minimum width=2.6cm, minimum height=1.2cm, align=center, fill=emsimaroon!10, thick, rounded corners] at (11.5, -2.5) {
        \textbf{Médias locaux} \\ \rule{2.2cm}{0.4pt} \\
        CVs \& Logos
    };

    % Services Externes
    % Google / GitHub OAuth
    \node (oauth) [draw, rectangle, minimum width=3.2cm, minimum height=1.4cm, align=center, fill=orange!10, thick, rounded corners] at (0, 3.0) {
        \textbf{Google / GitHub} \\ \rule{2.8cm}{0.4pt} \\
        OAuth 2.0 Auth API
    };
    
    % Serveur SMTP Gmail
    \node (smtp) [draw, rectangle, minimum width=3.2cm, minimum height=1.4cm, align=center, fill=purple!10, thick, rounded corners] at (0, -3.0) {
        \textbf{SMTP Gmail} \\ \rule{2.8cm}{0.4pt} \\
        Notifications E-mail \\
        Port 587 (TLS)
    };

    % Connecteurs avec routage propre (sans chevauchement)
    \draw[<->, >=latex, thick] (browser) -- (nginx) node[midway, above, sloped, fill=white, inner sep=1.5pt] {{\scriptsize HTTP, HTTPS}};
    
    % Nginx to Django: orthogonal par le haut
    \draw[<->, >=latex, thick] (nginx.east) -| (django.north) node[pos=0.3, below, fill=emsiblue!5, inner sep=1.5pt] {{\scriptsize Static}};
    
    \draw[<->, >=latex, thick] (nginx) -- (gunicorn) node[midway, left, fill=emsiblue!5, inner sep=1.5pt] {{\scriptsize Proxy}};
    
    % Gunicorn to Django: orthogonal par le bas
    \draw[<->, >=latex, thick] (gunicorn.east) -| (django.south) node[pos=0.3, above, fill=emsiblue!5, inner sep=1.5pt] {{\scriptsize WSGI}};
    
    \draw[<->, >=latex, thick] (django) -- (sqlite) node[midway, right, fill=emsiblue!5, inner sep=1.5pt] {{\scriptsize ORM SQLite}};
    \draw[<->, >=latex, thick] (django) -- (storage) node[midway, right, fill=emsiblue!5, inner sep=1.5pt] {{\scriptsize I/O Média}};
    \draw[<->, >=latex, thick, dashed] (browser) -- (oauth) node[midway, left, fill=white, inner sep=1.5pt] {{\scriptsize Redirect, Callback}};
    
    % Django to SMTP: contourne Gunicorn par le bas
    \draw[->, >=latex, thick, dashed] (django.south) |- (smtp.east) node[pos=0.7, below, fill=white, inner sep=1.5pt] {{\scriptsize Envoi Mail}};
    
    % Django to OAuth: contourne Nginx par le haut
    \draw[->, >=latex, thick, dashed] (django.north) |- (oauth.east) node[pos=0.7, above, fill=white, inner sep=1.5pt] {{\scriptsize Token API}};
\end{tikzpicture}
}
\caption{Diagramme de déploiement physique du système sur Oracle Cloud}
...
La base de données relationnelle (SQLite / MySQL) est structurée pour assurer une intégrité absolue et un temps d'accès optimal. Les tables fondamentales sont listées ci-dessous avec leurs clés primaires (PK) et étrangères (FK) :
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item ice : VARCHAR(15) NULL (Identifiant Commun de l'Entreprise)
